...
While getOPTS ":dfirRvels-:" opt ; do
	...
	- ) case $OPTARG in
		...
		recursive )	opt_recrusive=1 ;;
		verbose )	opt_verbose=1 ;;
		help ) /bin/rm	--help
		    echo "rm_secure:"
		    echo " -e --empty vider la corbeille"
		    echo " -l --list voir les fichier sauvé "
		    echo " -s --restore récuperer les fichiers"
		    return 0 ;;
	...
	shift $(($OPTIND - 1))
    #Créer éventuellement le répértoire
    ...
    while [ -n "$1" ] ; do 
    #Supression interactive : interroger l'utilisateur
